/**
 * ITCAST WEB
 * Created by zhousg on 2016/12/2.
 */
$(function(){
    banner();
    /*需要自己初始化工具提示*/
...
var banner = function(){
    /*
    *1.假设一组数据，把这个数据当作是后台给的模拟。  两种格式的图片路径
    *2.判断当前设备   宽度
    *3.根据设备渲染成所需要的样式  移动端使用图片 pc使用背景 (html格式的字符串)
    *3.1 动态创建elements元素
    *3.2 字符串动态拼接
    *3.3 模版引擎渲染 复杂业务逻辑的时候 （underscore template html格式的字符串）
     *目的把数据json格式的数据转化成html结构的字符串
    *4.渲染到页面当中  innerHTML html()
    *
    *5.测试 改变浏览器宽度  重新渲染  resize 页面尺寸发生改变触发
    * */
